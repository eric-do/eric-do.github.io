...
This post assumes you have an AWS account and can use the interface to create an AMI instance. Following the creation of the instance though, you'll next want to login using the command line.

## Set up access to instance via command line
This part's a little confusing if you make assumptions about how to get in. You should be logging in to the server as ec2-user, not your Amazon username.
...
### Potential issues
For whatever reason, the key Amazon gives you may be considered to be too insecure.
```
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
### Update key 
First fix is to try using chmod to change file permissions.
```
chmod 400 mykey.pem
```
...
# Run app
Everything is now set up on our EC2 instance. Next step is to build and run the app.
...
*Note: this can potentially be automated from a script, but it'd require accounting for asyncronous startup of the app container and database container, i.e. you can't begin seeding until your database has finished initialization.*

Your app should now be running! If it's not, double check:
